...
export default function AttendanceRecords() {
  ...
  // Get the first module's ID as default for the query, or use courseFilter
  const selectedModuleId = courseFilter === "all" ? modules[0]?.id : courseFilter
  const { data, isLoading } = useFetchAttendanceRecords(selectedModuleId, courseFilter !== "all")

  const allRecords: FlatRecord[] = useMemo(() => {
    return (data?.data ?? []).map((r: AttendanceRecordItem) => ({
